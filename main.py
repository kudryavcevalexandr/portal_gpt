...
-}
+SIGNAL_ZONES = [(0, 60), (60, 120), (121, 170)]
+YELLOW_DURATION = 2
+RED_DURATION = 5
 ...
 <TimerScreen>:
     ...
     Label:
         ...
         font_size: "32sp"
@@ -81,227 +76,195 @@ class TimerScreen(BoxLayout):
...
+            elif signal == "yellow":
+                self.bg_color = (0.96, 0.85, 0.17, 1)
             else:
...
+    @staticmethod
+    def _interval_overlaps(existing, candidate):
+        return candidate["start"] < existing["end"] and existing["start"] < candidate["end"]
+
+    def _try_place_interval(self, intervals, zone_start, zone_end, signal_type, duration, attempts=500):
+        latest_start = zone_end - duration
+        if latest_start < zone_start:
+            return False
+
+        for _ in range(attempts):
+            start = random.randint(zone_start, latest_start)
+            candidate = {"type": signal_type, "start": start, "end": start + duration}
+            if any(self._interval_overlaps(ex, candidate) for ex in intervals):
                 continue
...
+        return False
+
+    def _generate_signal_timeline(self):
+        intervals = []
+        for zone_start, zone_end in SIGNAL_ZONES:
+            if not self._try_place_interval(intervals, zone_start, zone_end, "red", RED_DURATION):
+                return [], {}
+            for _ in range(6):
+                if not self._try_place_interval(intervals, zone_start, zone_end, "yellow", YELLOW_DURATION):
+                    return [], {}
+
+        signal_by_second = {}
+        for interval in intervals:
+            for second in range(interval["start"], interval["end"]):
+                signal_by_second[second] = interval["type"]
+        return sorted(intervals, key=lambda x: x["start"]), signal_by_second
+
+    def _trigger_signal(self, signal_type):
+        if signal_type == "red":
+            self.status_text = "СЕРИЯ: красный сигнал 5 секунд"
             self._play(self.attack_sound or self.punch_sound)
         else:
-            self.status_text = "КОНТРОЛЬ: джеб → оттяжка → двойка → выход"
+            self.status_text = "ДВОЙКА: жёлтый сигнал 2 секунды"
...
+                for _ in range(10):
+                    self.signal_intervals, self.signal_by_second = self._generate_signal_timeline()
...
+            if not self.is_break and self.current_round >= 3:
+                signal = self.signal_by_second.get(elapsed)
+                prev_signal = self.signal_by_second.get(elapsed - 1)
+                if signal and signal != prev_signal:
+                    self._trigger_signal(signal)
+                elif not signal:
+                    self.status_text = "БЕЛЫЙ: движение и контроль"
             self.time_left -= 1
             self.update_display()
         else:
             self.next_step()
